...
test('MCP server exposes BoardForge tools and runs controlled jobs', async () => {
  ...
  try {
    ...
    assert.equal(listed.tools.some((tool) => tool.name === 'list_universal_board_scenarios'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'classify_board_complexity'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'recommend_adaptive_stackup'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'score_routability'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'detect_constraint_conflicts'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'decide_structural_regeneration'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'detect_template_reuse'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'generate_board_specific_report'), true)
    ...
    assert.equal(listed.tools.some((tool) => tool.name === 'autotrace_board'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'autotrace_diff_pairs'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'calculate_trace_width'), true)
    assert.equal(listed.tools.some((tool) => tool.name === 'validate_via_manufacturability'), true)
