...
function validateRoute(route, board, components, profile, pads = []) {
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    for (const pad of pads) {
      if (!pad.x || !pad.y || pad.netName === route.net) continue
...
function validateDifferentialPairs(routes) {
  ...
  for (const route of routes.filter((item) => ['USB_DIFF', 'ETHERNET_DIFF', 'CAN_DIFF'].includes(item.className || ''))) {
    ...
    if (mate && route.status === 'routed' && mate.status === 'routed') {
      ...
      if (mismatch > 1.5) issues.push(issue('ERROR', 'DIFF_PAIR_SKEW_TOO_HIGH', `${route.net}/${mateName} length mismatch is ${round(mismatch)}mm.`, { net: route.net, mate: mateName, mismatchMm: round(mismatch) }))
      if ((route.layerPreference?.[0] || 'F.Cu') !== (mate.layerPreference?.[0] || 'F.Cu')) issues.push(issue('ERROR', 'DIFF_PAIR_LAYER_MISMATCH', `${route.net}/${mateName} are not on the same primary layer.`, { net: route.net, mate: mateName }))
    }
  }
  return issues
}
    ...
    for (let b = a + 1; b < routes.length; b += 1) {
      ...
      for (const segA of routeSegments(routes[a])) {
        for (const segB of routeSegments(routes[b])) {
          if (segmentsIntersect(segA.start, segA.end, segB.start, segB.end)) issues.push(issue('ERROR', 'ROUTE_ROUTE_CLEARANCE', `${routes[a].net} intersects ${routes[b].net} on the same layer.`, { nets: [routes[a].net, routes[b].net], clearanceMm: clearance }))
        }
      }
    }
  }
...
function segmentStaysInsideBoard(start, end, outline) {
  ...
  if (!pointInPolygon(start, outline) || !pointInPolygon(end, outline)) return false
