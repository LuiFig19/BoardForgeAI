...
export const boardTemplates = {
  ...
  ESP32_S3_SENSOR: {
    id: 'ESP32_S3_SENSOR', name: 'ESP32-S3 Sensor Board', dimensionsMm: { width: 96, height: 64 }, layerCount: 4,
    outline: (w, h) => roundedRectanglePoints(w, h, 4), mountingHoles: (w, h) => cornerHoles(w, h, 2.7, 6),
  ...
  ESP32_S3_POE_SENSOR: {
    id: 'ESP32_S3_POE_SENSOR', name: 'ESP32-S3 PoE Ethernet Sensor', dimensionsMm: { width: 150, height: 90 }, layerCount: 4,
    outline: (w, h) => roundedRectanglePoints(w, h, 4), mountingHoles: (w, h) => cornerHoles(w, h, 3, 8),
  ...
  ROBOTICS_CONTROLLER: {
    id: 'ROBOTICS_CONTROLLER', name: 'Robotics Controller', dimensionsMm: { width: 132, height: 82 }, layerCount: 4,
    outline: (w, h) => roundedRectanglePoints(w, h, 4), mountingHoles: (w, h) => cornerHoles(w, h, 3, 7),
    requiredNetClasses: ['BATTERY', 'POWER_LOW_CURRENT', 'CAN_DIFF', 'RS485_DIFF', 'I2C', 'UART', 'GROUND', 'SENSOR'],
    componentGroups: ['MCU', 'CAN_TRANSCEIVER', 'RS485_TRANSCEIVER', 'FIELD_CONNECTOR', 'MOTOR_HEADERS', 'POWER_INPUT', 'REGULATOR', 'ENCODER_INPUTS', 'DEBUG'],
    validationChecks: ['connectors_accessible', 'can_pair_classified', 'power_entry_clear'],
    exportRequirements: ['kicad_project', 'drc_report', 'gerbers', 'drill', 'bom', 'cpl', 'manufacturing_review'],
  },
  INDUSTRIAL_IO: {
    id: 'INDUSTRIAL_IO', name: 'Industrial I/O Controller', dimensionsMm: { width: 150, height: 86 }, layerCount: 4,
    outline: (w, h) => roundedRectanglePoints(w, h, 4), mountingHoles: (w, h) => cornerHoles(w, h, 3.2, 8),
    requiredNetClasses: ['POWER_LOW_CURRENT', 'POWER_HIGH_CURRENT', 'HIGH_VOLTAGE', 'ISOLATION_BOUNDARY', 'CAN_DIFF', 'RS485_DIFF', 'GROUND'],
    componentGroups: ['TERMINAL_BLOCK', 'TVS', 'ISOLATOR', 'RELAY_OR_DRIVER', 'CAN_TRANSCEIVER', 'RS485_TRANSCEIVER', 'MCU', 'POWER_INPUT', 'REGULATOR', 'DEBUG'],
    validationChecks: ['terminal_blocks_on_edge', 'field_protection_near_connector', 'isolation_boundary_clear', 'wide_clearance_field_io'],
    exportRequirements: ['kicad_project', 'drc_report', 'gerbers', 'drill', 'bom', 'cpl', 'manufacturing_review'],
  },
