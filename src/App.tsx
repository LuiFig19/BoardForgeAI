...
function LandingPage() {
  ...
  const problemPairs = {
    without: ['random file edits', 'missing footprints', 'broken Edge.Cuts', 'off-board components', 'fake DRC confidence', 'manual export cleanup'],
    with: ['structured JSON tool calls', 'KiCad project scanning', 'controlled board writing', 'net class rules', 'routing checks', 'ERC/DRC parsing', 'manufacturing package validation'],
  }
  const workflow = [
    {
      title: 'Prompt Codex',
      body: '"Design this PCB..." Codex receives the product intent, constraints, interfaces, and review requirements.',
      details: ['natural language request', 'saved board outline', 'explicit constraints'],
      icon: TerminalSquare,
    },
    {
      title: 'BoardForge Plugin',
      body: 'Codex calls controlled BoardForge tools with structured JSON instead of directly mutating KiCad project files.',
      details: ['schema-validated jobs', 'approved workspace', 'tool-call audit trail'],
      icon: Plug,
    },
    {
      title: 'Local KiCad Tool Server',
      body: 'The local helper creates projects, writes Edge.Cuts, scans libraries, and runs whitelisted KiCad automation on your machine.',
      details: ['KiCad file writes', 'library scanning', 'local CLI calls'],
      icon: Server,
    },
    {
      title: 'Validation Gates',
      body: 'ERC, DRC, fab rules, placement checks, routing checks, and package validation run before manufacturing handoff.',
      details: ['DRC scanner', 'placement legality', 'fab-rule checks'],
      icon: ShieldCheck,
    },
    {
      title: 'Manufacturing Outputs',
      body: 'When checks are reviewed, BoardForge packages real Gerbers, drill files, BOM, CPL, JLCPCB package data, and review reports.',
      details: ['Gerbers and drill', 'BOM / CPL', 'human review report'],
      icon: PackageCheck,
    },
  ]
  const previewBoards: Array<{
    name: string
    body: string
  ...
  }> = [
    {
      ...
      body: 'RJ45 edge connector, USB-C service port, ESP32 region, sensor header, and four mounting holes.',
      request: {
        ...emptyRequest,
        ...
        mcu: 'ESP32-S3-WROOM',
        connectors: 'RJ45 edge, USB-C service, sensor header',
      },
      footprints: [
        { id: 'poe_rj45', packageId: 'rj45-mag', ref: 'J1', x: 0.86, y: 0.5, rotation: -90, side: 'top' },
        { id: 'poe_usb', packageId: 'usb-c-mid', ref: 'J2', x: 0.15, y: 0.58, rotation: 90, side: 'top' },
        { id: 'poe_mcu', packageId: 'qfn-56', ref: 'U1', x: 0.5, y: 0.5, rotation: 0, side: 'top' },
        { id: 'poe_pwr', packageId: 'l-4x4', ref: 'L1', x: 0.66, y: 0.28, rotation: 0, side: 'top' },
        { id: 'poe_sensor', packageId: 'soic-8', ref: 'U3', x: 0.34, y: 0.34, rotation: 0, side: 'top' },
        { id: 'poe_tp1', packageId: 'tp-1mm', ref: 'H1', x: 0.16, y: 0.2, rotation: 0, side: 'top' },
        { id: 'poe_tp2', packageId: 'tp-1mm', ref: 'H2', x: 0.84, y: 0.2, rotation: 0, side: 'top' },
        { id: 'poe_tp3', packageId: 'tp-1mm', ref: 'H3', x: 0.16, y: 0.8, rotation: 0, side: 'top' },
        { id: 'poe_tp4', packageId: 'tp-1mm', ref: 'H4', x: 0.84, y: 0.8, rotation: 0, side: 'top' },
      ],
      routes: [
        { id: 'poe_eth', layer: 'F.Cu', net: 'ETH', width: 'signal', points: [[0.86, 0.5], [0.7, 0.5], [0.7, 0.46], [0.5, 0.46]] },
        { id: 'poe_usb', layer: 'F.Cu', net: 'USB', width: 'signal', points: [[0.15, 0.58], [0.3, 0.58], [0.3, 0.5], [0.5, 0.5]] },
        { id: 'poe_power', layer: 'F.Cu', net: 'VIN', width: 'power', points: [[0.66, 0.28], [0.66, 0.42], [0.56, 0.42], [0.5, 0.5]] },
      ],
      vias: [{ id: 'poe_v1', x: 0.46, y: 0.38, net: 'GND' }, { id: 'poe_v2', x: 0.72, y: 0.62, net: 'ETH' }],
    },
    {
      name: 'USB-C sensor board',
      body: 'Compact rounded board with USB-C, MCU, I2C sensor connector, and regulator section.',
      request: {
        ...emptyRequest,
        projectName: 'USB-C sensor board',
      ...
      footprints: [
        { id: 'usb_usb', packageId: 'usb-c-mid', ref: 'J1', x: 0.14, y: 0.5, rotation: 90, side: 'top' },
        { id: 'usb_mcu', packageId: 'qfn-56', ref: 'U1', x: 0.46, y: 0.5, rotation: 45, side: 'top' },
        { id: 'usb_sensor', packageId: 'soic-8', ref: 'U2', x: 0.7, y: 0.42, rotation: 0, side: 'top' },
        { id: 'usb_reg', packageId: 'l-4x4', ref: 'U3', x: 0.75, y: 0.66, rotation: 0, side: 'top' },
        { id: 'usb_c1', packageId: 'c-0603', ref: 'C1', x: 0.34, y: 0.68, rotation: 0, side: 'top' },
        { id: 'usb_r1', packageId: 'r-0603', ref: 'R1', x: 0.58, y: 0.72, rotation: 0, side: 'top' },
      ],
      routes: [
        { id: 'usb_dp', layer: 'F.Cu', net: 'USB', width: 'signal', points: [[0.14, 0.5], [0.3, 0.5], [0.46, 0.5]] },
        { id: 'usb_i2c', layer: 'F.Cu', net: 'I2C', width: 'signal', points: [[0.46, 0.5], [0.58, 0.42], [0.7, 0.42]] },
        { id: 'usb_3v3', layer: 'F.Cu', net: '+3V3', width: 'power', points: [[0.75, 0.66], [0.62, 0.66], [0.46, 0.5]] },
      ],
      vias: [{ id: 'usb_v1', x: 0.38, y: 0.34, net: 'GND' }, { id: 'usb_v2', x: 0.82, y: 0.52, net: '+3V3' }],
    },
    {
      name: 'Robotics controller',
      body: 'Terminal blocks, CAN/UART headers, motor and sensor connectors, and defined power input region.',
      request: {
        ...emptyRequest,
        projectName: 'Robotics controller',
        boardType: 'robotics controller',
        boardWidthMm: 85,
        boardHeightMm: 55,
        boardSize: '85 mm x 55 mm',
        mcu: 'STM32G4',
        connectors: 'terminal blocks, CAN JST, UART headers, motor connectors',
      },
      footprints: [
        { id: 'bot_power', packageId: 'rj45-mag', ref: 'J1', x: 0.82, y: 0.32, rotation: -90, side: 'top' },
        { id: 'bot_motor', packageId: 'usb-c-mid', ref: 'J2', x: 0.84, y: 0.7, rotation: -90, side: 'top' },
        { id: 'bot_mcu', packageId: 'qfn-56', ref: 'U1', x: 0.43, y: 0.5, rotation: 0, side: 'top' },
        { id: 'bot_can', packageId: 'soic-8', ref: 'U2', x: 0.28, y: 0.32, rotation: 0, side: 'top' },
        { id: 'bot_reg', packageId: 'l-4x4', ref: 'L1', x: 0.62, y: 0.28, rotation: 0, side: 'top' },
        { id: 'bot_tp1', packageId: 'tp-1mm', ref: 'CAN', x: 0.2, y: 0.68, rotation: 0, side: 'top' },
        { id: 'bot_tp2', packageId: 'tp-1mm', ref: 'UART', x: 0.3, y: 0.72, rotation: 0, side: 'top' },
      ],
      routes: [
        { id: 'bot_power', layer: 'F.Cu', net: 'VIN', width: 'power', points: [[0.82, 0.32], [0.66, 0.32], [0.62, 0.28], [0.48, 0.44]] },
        { id: 'bot_can', layer: 'F.Cu', net: 'CAN', width: 'signal', points: [[0.28, 0.32], [0.36, 0.4], [0.43, 0.5]] },
        { id: 'bot_motor', layer: 'F.Cu', net: 'PWM', width: 'signal', points: [[0.43, 0.5], [0.62, 0.62], [0.84, 0.7]] },
      ],
      vias: [{ id: 'bot_v1', x: 0.5, y: 0.35, net: 'VIN' }, { id: 'bot_v2', x: 0.22, y: 0.52, net: 'CAN' }, { id: 'bot_v3', x: 0.68, y: 0.72, net: 'GND' }],
    },
    {
      name: 'Drone flight controller',
      body: 'Square 30.5 mm mounting pattern with USB-C edge, MCU/IMU region, and UART pads.',
      request: {
        ...emptyRequest,
        projectName: 'Drone flight controller',
        ...
        mechanicalConstraints: '30.5 mm mounting pattern, centered IMU',
      },
      footprints: [
        { id: 'drone_usb', packageId: 'usb-c-mid', ref: 'J1', x: 0.14, y: 0.5, rotation: 90, side: 'top' },
        { id: 'drone_mcu', packageId: 'qfn-56', ref: 'U1', x: 0.52, y: 0.48, rotation: 45, side: 'top' },
        { id: 'drone_imu', packageId: 'soic-8', ref: 'U2', x: 0.53, y: 0.28, rotation: 0, side: 'top' },
        { id: 'drone_reg', packageId: 'l-4x4', ref: 'U3', x: 0.74, y: 0.56, rotation: 0, side: 'top' },
        { id: 'drone_uart1', packageId: 'tp-1mm', ref: 'T1', x: 0.3, y: 0.78, rotation: 0, side: 'top' },
        { id: 'drone_uart2', packageId: 'tp-1mm', ref: 'T2', x: 0.42, y: 0.8, rotation: 0, side: 'top' },
        { id: 'drone_uart3', packageId: 'tp-1mm', ref: 'T3', x: 0.54, y: 0.8, rotation: 0, side: 'top' },
        { id: 'drone_uart4', packageId: 'tp-1mm', ref: 'T4', x: 0.66, y: 0.78, rotation: 0, side: 'top' },
      ],
      routes: [
        { id: 'drone_usb', layer: 'F.Cu', net: 'USB', width: 'signal', points: [[0.14, 0.5], [0.32, 0.5], [0.52, 0.48]] },
        { id: 'drone_imu', layer: 'F.Cu', net: 'SPI', width: 'signal', points: [[0.52, 0.48], [0.52, 0.36], [0.53, 0.28]] },
        { id: 'drone_power', layer: 'F.Cu', net: '+5V', width: 'power', points: [[0.74, 0.56], [0.64, 0.56], [0.52, 0.48]] },
      ],
      vias: [{ id: 'drone_v1', x: 0.24, y: 0.24, net: 'GND' }, { id: 'drone_v2', x: 0.76, y: 0.24, net: 'GND' }, { id: 'drone_v3', x: 0.24, y: 0.76, net: 'GND' }, { id: 'drone_v4', x: 0.76, y: 0.76, net: 'GND' }],
    },
  ]
  const capabilities = [
    ['Create KiCad projects', 'Start local project folders through controlled plugin jobs.', FolderKanban],
    ['Generate board outlines', 'Turn browser outlines into exact Edge.Cuts intent.', CircuitBoard],
    ['Write Edge.Cuts', 'Apply reviewed outline geometry to KiCad board files.', Layers3],
    ['Assign footprints', 'Map components to KiCad/JLCPCB footprint candidates.', Database],
    ['Check 3D models', 'Report missing STEP/WRL coverage before review.', CircuitBoard],
    ['Plan placement', 'Stage legal component regions and constraints.', Workflow],
    ['Create net classes', 'Classify power, default, and high-speed nets.', GitBranch],
    ['Route/review copper', 'Plan and inspect routing work with DRC gates.', Workflow],
    ['Run ERC/DRC', 'Call KiCad checks through whitelisted local actions.', ShieldCheck],
    ['Parse reports', 'Convert KiCad reports into actionable repair plans.', FileCheck2],
    ['Plan safe repairs', 'Suggest scoped fixes before writing changes.', ShieldAlert],
    ['Export Gerbers', 'Export fabrication data from real local KiCad projects.', Factory],
    ['Export drill files', 'Package drill outputs after validation.', Download],
    ['Export BOM', 'Create bill-of-materials handoff files.', FileArchive],
    ['Export CPL', 'Generate pick-and-place data for review.', PackageCheck],
    ['Package JLCPCB files', 'Validate the package contents and file names.', PackageCheck],
    ['Generate review reports', 'Summarize remaining risks for human signoff.', FileCheck2],
  ] as const
  const supportedBoards = ['Embedded controllers', 'ESP32 / IoT boards', 'USB devices', 'Ethernet / PoE boards', 'Robotics controllers', 'Motor controllers', 'LED controllers', 'Battery / charger boards', 'Industrial I/O', 'Sensor boards', 'Dev boards', 'Adapter boards', 'Drone flight controllers', 'Custom-shaped boards']
  const safetyCards = [
    ['Structured JSON commands', 'Every plugin job is schema-shaped before execution.', TerminalSquare],
    ...
    ['Fab-rule checks', 'Manufacturer profile checks are explicit review gates.', Factory],
  ...
  const examplePrompts = [
    'Use BoardForge to create a 65mm x 40mm ESP32-S3 sensor board with USB-C, 3V3 regulator, I2C header, and four M3 mounting holes.',
    'Use BoardForge to scan this KiCad project, find missing footprints, run DRC, and generate a manufacturing review.',
    'Use BoardForge to route the critical nets first, create net classes, and report anything that fails DRC.',
    'Use BoardForge to package this project for JLCPCB with Gerbers, drill files, BOM, and CPL.',
  ]
    ...
    <main className="landing-page">
      <section className="hero-section">
        <div className="hero-copy">
          ...
          <p>
            BoardForge gives Codex safe local tools to create, inspect, route, validate, and export real KiCad PCB projects - without random file edits.
          </p>
          <div className="action-row">
            <a className="primary-action" href="#/plugin">
              <Plug size={18} /> Get BoardForge Plugin
            </a>
            <a className="secondary-action" href="#/generate">
              <Play size={18} /> Open Board Builder
            </a>
      ...
      <section className="landing-section problem-section">
        <div className="section-heading">
          <StatusBadge tone="amber">the problem</StatusBadge>
          <h2>AI can write files. PCB engineering needs rules.</h2>
          <p>Codex can edit projects and run commands, but PCB work needs controlled tooling, constraints, validation, routing rules, net classes, layer rules, and manufacturing checks.</p>
        </div>
        <div className="comparison-grid">
          <article>
            <h3>Without BoardForge</h3>
            {problemPairs.without.map((item) => <span key={item}><X size={16} /> {item}</span>)}
          </article>
          <article>
            <h3>With BoardForge</h3>
            {problemPairs.with.map((item) => <span key={item}><CheckCircle2 size={16} /> {item}</span>)}
          </article>
        </div>
      </section>
      ...
      <section className="render-gallery-section">
        <div className="section-heading">
          <StatusBadge tone="green">browser command center</StatusBadge>
          <h2>Preview in the browser. Execute locally in KiCad.</h2>
          <p>The website helps you plan, preview, and manage projects. The BoardForge Plugin performs real KiCad work on your machine.</p>
        </div>
        <div className="render-gallery-grid">
          {previewBoards.map((board) => (
            <article className="render-card" key={board.name}>
              <RealisticPcbViewer request={board.request} footprints={board.footprints} routes={board.routes} vias={board.vias} interactive={false} autoRotate />
      ...
      <section className="scroll-workflow landing-workflow" id="workflow">
      ...
      <section className="landing-section">
        <div className="section-heading">
          <StatusBadge tone="cyan">capabilities</StatusBadge>
          <h2>One plugin. Real PCB workflows.</h2>
          <p>BoardForge gives Codex controlled PCB tools: create projects, write board outlines, classify nets, validate placement, review routing, run ERC/DRC, and package manufacturing files.</p>
        </div>
      ...
      <section className="landing-section builder-feature">
        <div>
          <StatusBadge tone="green">board builder</StatusBadge>
        ...
        <div className="builder-preview" aria-label="Board builder preview">
          <div className="builder-toolbar">
            <span>custom outline</span>
            <button type="button">Export KiCad Outline</button>
          </div>
          <svg viewBox="0 0 320 190" role="img" aria-label="Rounded custom PCB outline with mounting holes">
            <path d="M42 42 Q42 24 60 24 H252 Q274 24 274 46 V136 Q274 162 248 162 H72 Q42 162 42 132 Z" />
            {[68, 248].map((x) => [50, 136].map((y) => <circle key={`${x}-${y}`} cx={x} cy={y} r="9" />))}
            <rect x="118" y="74" width="72" height="46" rx="5" />
            <rect x="46" y="86" width="42" height="28" rx="4" />
            <rect x="214" y="74" width="42" height="24" rx="3" />
            <polyline points="88,100 118,96 154,96 214,86" />
            <polyline points="154,120 198,134 236,134" />
          </svg>
          <div className="builder-prompt">
            <TerminalSquare size={16} />
            <code>Use BoardForge to create KiCad Edge.Cuts from this saved outline.</code>
          </div>
        </div>
      </section>
      ...
      <section className="landing-section">
        <div className="section-heading">
          <StatusBadge tone="cyan">supported boards</StatusBadge>
          <h2>Built for all PCB engineers, not one niche.</h2>
        </div>
        <div className="supported-grid">
          {supportedBoards.map((board) => <span key={board}>{board}</span>)}
        </div>
      </section>
      ...
      <section className="landing-section safety-section">
        <div className="section-heading">
          ...
          <h2>Controlled AI. Real validation.</h2>
      ...
      <section className="landing-section install-section">
        <div className="section-heading">
          <StatusBadge tone="green">plugin beta coming soon</StatusBadge>
      ...
      <section className="landing-section" id="prompts">
        <div className="section-heading">
          <StatusBadge tone="cyan">example prompts</StatusBadge>
          <h2>What you can say in Codex.</h2>
        </div>
        <div className="example-prompt-grid">
          {examplePrompts.map((prompt) => <code key={prompt}>{prompt}</code>)}
        </div>
      </section>

      <section className="final-cta">
        <StatusBadge tone="green">local KiCad execution</StatusBadge>
        <h2>Build PCB projects with Codex, safely.</h2>
        <p>BoardForge turns AI prompting into structured KiCad project execution, validation, and manufacturing export workflows.</p>
        <div className="action-row">
          <a className="primary-action" href="#/plugin"><Plug size={18} /> Get BoardForge Plugin</a>
          <a className="secondary-action" href="#/generate"><CircuitBoard size={18} /> Open Board Builder</a>
          <a className="secondary-action" href="#/docs"><ArrowRight size={18} /> View Docs</a>
        </div>
      </section>
    </main>
  )
